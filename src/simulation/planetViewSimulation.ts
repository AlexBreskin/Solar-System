...
for (const id of Object.keys(VISUAL_CONFIG.moonOrbitalRadii)) {
  ...
  MOON_PARENT_MAP[id] = body.parent!;
...
export function computeScaledLayout(
  ...
  const moonDiameters = moons.map((m) => bodies[m].diameter);
  ...
  const realDistances = sortedMoons.map((m) => bodies[m].distanceFromParent);
  ...
  for (let i = 1; i < sortedMoons.length; i++) {
    ...
    if (localMoonOrbitalRadii[outer] - localMoonOrbitalRadii[inner] < minGap) {
      localMoonOrbitalRadii[outer] = localMoonOrbitalRadii[inner] + minGap;
    }
  }
  const outermostR = localMoonOrbitalRadii[sortedMoons[sortedMoons.length - 1]];
...
export class PlanetViewSimulation {
  ...
  advanceAngles(dt: number, speed: number, moons: string[]): void {
    const fastestPeriod = moons.length
      ? Math.min(...moons.map((m) => Math.abs(this.bodies[m].orbitalPeriod)))
      : 27.32;
    ...
    for (const moonId of moons) {
      ...
      this.angles[moonId] += relSpeed * baseSpeed * dt;
    }
  }
  ...
  updatePositions(
    ...
    for (const moonId of moons) {
      ...
      const r = radii[moonId];
      const angle = this.angles[moonId];
