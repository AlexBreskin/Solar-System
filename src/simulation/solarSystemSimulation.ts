...
export class SolarSystemSimulation {
  ...
  constructor(bodies: Record<string, CelestialBody>, visualConfig: VisualConfig) {
    ...
    this.fixedIds = Object.keys(bodies).filter(
      id => ROOT_BODY_TYPES.has(bodies[id].type) || bodies[id].type === BodyType.Belt,
    );
    ...
    this.angles = Object.fromEntries(this.bodyIds.map((id, i) => [id, i * 0.7]));
    this.positions = Object.fromEntries(this.bodyIds.map(id => [id, { x: 0, y: 0 }]));
  }
  ...
  updatePositions(cx: number, cy: number): void {
    ...
    for (const id of this.planetIds) {
      ...
      pos[id] = { x: cx + r * Math.cos(this.angles[id]), y: cy + r * Math.sin(this.angles[id]) };
    }
    for (const id of this.moonIds) {
      ...
      pos[id] = { x: px + pr * Math.cos(this.angles[id]), y: py + pr * Math.sin(this.angles[id]) };
    }
    for (const id of this.moonIds) {
